...
test('Should login the user into account that was created earlier', async () => {
     const response = await axios.post("https://ifback.onrender.com/api/login",{email : "testUser@gmail.com" , password : "testpass1"})
...
test('Should log out the user out of account', async () => {
    const response = await axios.post("https://ifback.onrender.com/api/logout")
    expect(response.status).toBe(200)
