...
test('Should return the posts sorted according to algorithm(based on save posts)', async () => {
     const user = await axios.post("https://ifback.onrender.com/api/login",{email : "testUser@gmail.com" , password : "testpass1"})
     const lastSavedPost = await axios.get(`https://ifback.onrender.com/api/post/${user.data.user.postsSaved[0]}`)
