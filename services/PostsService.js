        ...
        try {
          if (isAuth && user) {
            if (user.postsSaved && user.postsSaved.length > 0) {
              const lastSavedPost = await axios.get(`https://inkfindercl.vercel.app/api/post/${user.postsSaved[user.postsSaved.length-1]}`);
              const preference = lastSavedPost.data.prompt.split(',')[lastSavedPost.data.prompt.split(',').length - 1]?.trim();
              const response = await axios.post('https://inkfindercl.vercel.app/api/posts', {preference}, {
                headers: {
